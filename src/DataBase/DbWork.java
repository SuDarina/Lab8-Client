...
public class DbWork {
    ...
    public int getMaxColor() throws SQLException{
        try(PreparedStatement statement = connection.prepareStatement("SELECT * FROM Users")) {
            ResultSet rs = statement.executeQuery();
            int max = -1;
            while (rs.next()) {
                int res = rs.getInt("color");
                if (res > max) {
                    max = res;
                    idColor = max;
                }
    ...
    public int getColor(User user) throws SQLException {
    ...
    public void regester (User user) throws SQLException {
        idColor = getMaxColor();
        try(PreparedStatement statement = connection.prepareStatement("INSERT INTO Users (id, login, password, online, color) VALUES (DEFAULT , ?, ?, ?, ?)")){
            statement.setString(1, user.getLogin());
            statement.setString(2, user.getPassword());
